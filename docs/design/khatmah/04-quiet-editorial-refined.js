(() => {
  ...
  function setLanguage(next, preservePosition = false) {
    const hashTarget = preservePosition && location.hash ? document.getElementById(decodeURIComponent(location.hash.slice(1))) : null;
    ...
    document.querySelectorAll('[data-media]').forEach(element => { element.src = 'staged-media/' + media[next][element.dataset.media]; });
    document.querySelectorAll('[data-full-media]').forEach(element => { element.href = 'staged-media/' + media[next][element.dataset.fullMedia]; });
  ...
  let saved;
  try { saved = localStorage.getItem('khatmah-design-language'); } catch (_) { /* Private browsing can deny storage. */ }
  const initial = saved === 'ar' || saved === 'en' ? saved : (navigator.language?.toLowerCase().startsWith('ar') ? 'ar' : 'en');
  setLanguage(initial);
  switcher.addEventListener('click', () => {
    const next = language === 'en' ? 'ar' : 'en';
    setLanguage(next, true);
    try { localStorage.setItem('khatmah-design-language', next); } catch (_) { /* The switch still works. */ }
  });
})();
